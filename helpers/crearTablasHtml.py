def crearTabla(dataframe, nombreTabla): #nombre tabla es el nombre que le pondre en html
    ...
    archivo=open(f"./tables/{nombreTabla}.html","w") #(cojame un espacio para guardarlo en html dandole la ruta donde lo quiero guardar) aqui estamos abriendo un archivo y creandolo en la carpeta tables para llevarlo al html - w es que le permite al usuario escribir (un permiso)
    ...
    <body>
    '''.encode('utf-8')
               with open('indext.html', 'wb') as archivo:
